...
public class SpeechParser {
	...
	public ParseResult parse(){
		...
		for (int i = 0; i <= (parseResult.length+1)/2 - 1; i++)
        {
	        ...
	        {
				if(parseResult[i].sTimePhrase.compareTo(parseResult[j].sTimePhrase) == 0){
					parseResult[i].iHit ++;
	...
	private ParseResult parseChinese(String speechText) {
		...
		final char[] chineseNumbers = new char[]{'零', '一', '二', '三', '四', '五', '六', '七', '八', '九'};
		...
		while(index != -1){
			...
			if(index >= 1){
				afterNumber = (arrayContain(chineseNumbers, speechText.charAt(index - 1)));
			}
			if(index + 1 < speechText.length()){
				beforeNumber = (arrayContain(chineseNumbers, speechText.charAt(index + 1)));
			}
			speechText = speechText.replaceFirst(String.valueOf(CHINESE_TEN),
					afterNumber? beforeNumber? "" : "0" : beforeNumber? "1": "10");
			
			index = speechText.indexOf(CHINESE_TEN);
		}
		for(int i = 0; i < arabicNumbers.length; i++){
			speechText = speechText.replace(chineseNumbers[i], arabicNumbers[i]);
		}
		...
		while(index != -1){
			...
			if(index + 1 < speechText.length()){
				if(speechText.charAt(index + 1) == '过'){
					beforeNumber = (Character.isDigit(speechText.charAt(index + 2)));
				}else{
			...
			if(afterNumber){
				if(beforeNumber){
					int start = speechText.charAt(index + 1) == '过'? index + 2: index + 1;
					int i = start;
					...
					try{
						cal.set(Calendar.MINUTE, Integer.valueOf(speechText.substring(start, i)));
					}catch(Exception e){
						Log.w(e.getMessage());
					}finally{
					}
				...
				try{
					cal.set(Calendar.HOUR, Integer.valueOf(speechText.substring(i+1, index)));
					isTimeSet = true;
					timePhrase_time = cal.get(Calendar.HOUR) + ":" + cal.get(Calendar.MINUTE);
				}catch(Exception e){
					Log.w(e.getMessage());
				}finally{
				}
		...
		while(index != -1){
			...
			if(afterNumber){
				if(beforeNumber){
					...
					try{
						cal.set(Calendar.DAY_OF_MONTH, Integer.valueOf(speechText.substring(start, i)));
					}catch(Exception e){
						Log.w(e.getMessage());
					}finally{
					}
				...
				try{
					cal.set(Calendar.MONTH, Integer.valueOf(speechText.substring(i+1, index))-1);
					isDateSet = true;
					timePhrase_date = (cal.get(Calendar.MONTH)+1) + "月" + cal.get(Calendar.DAY_OF_MONTH) + "日";
				}catch(Exception e){
					Log.w(e.getMessage());
				}finally{
				}
		...
		if(!isDateSet){
			...
			while(index != -1){
				...
				if(afterNumber){
					...
					try{
						cal.set(Calendar.DAY_OF_MONTH, Integer.valueOf(speechText.substring(i+1, index)));
						isDateSet = true;
		...
		while(index != -1){
			if(index + 1 < speechText.length() && Character.isDigit(speechText.charAt(index + 1))){
				int start = index + 1;
				int i = start;
				while(i < speechText.length() && 
						Character.isDigit(speechText.charAt(i))){
				...
				try{
					...
					if (j == 0){
						...
						timePhrase_week = "周" + chineseNumbers[cal.get(Calendar.DAY_OF_WEEK)-1];
					}
				}catch(Exception e){
					Log.w(e.getMessage());
				}finally{
		...
		if(speechText.indexOf("大后天") != -1){
			...
			final String[] timeUnit = new String[]{"分钟", "小时", "天", "周", "个月", "年"}; 
			final int[] timeUnitField = new int[]{Calendar.MINUTE, Calendar.HOUR, Calendar.DAY_OF_MONTH, Calendar.WEEK_OF_MONTH, Calendar.MONTH, Calendar.YEAR}; 
			for(int i = 0; i < timeUnit.length; i++){
				if(isTimeSet == true && i < 2)
					continue;
				index = speechText.indexOf(timeUnit[i] + "后");
				if(index >= 1 && Character.isDigit(speechText.charAt(index - 1))){
					int start = index - 1;
					while(start >= 0 && 
					...
					try{
						int j = Integer.valueOf(speechText.substring(start + 1, index));
